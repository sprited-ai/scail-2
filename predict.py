...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        negative_prompt: str = Input(default="", description="What to avoid, e.g. 'distorted limbs, camera movement, blurry'."),
        ...
        preset: str = Input(default="quality", choices=["quality", "fast"],
                            description="quality: official sampling (UniPC, 40 steps, CFG 5, shift 3). fast: the official ComfyUI recipe with the lightx2v step/CFG-distill LoRA (Euler, 6 steps, CFG 1, shift 5) — roughly 10x cheaper."),
        ...
        dpo_lora: float = Input(default=0, ge=0, le=2, description="Strength of the official Bias-Aware DPO LoRA (0 = off). The official ComfyUI template uses 1.0 together with the fast preset."),
        ...
        params = GraphParams(
            ...
            steps=steps or pr["steps"], cfg=guidance_scale if guidance_scale > 0 else pr["cfg"],
